    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (!endProgram)
            {
                ...
                PrintList(oddCountNums, "ODD");

                // Display numbers that occurred an even number of times in the user list.
                List<uint> evenCountNums = tuple.Item2;
                PrintList(evenCountNums, "EVEN");

                endProgram = EndProgramMenu(); // End program user menu.
            } // endwhile(!endProgram).

            Console.Write("\nThank you for using this program, press any key to exit Console.");
            Console.ReadKey();
        }
        ...
        static uint[] GetUserNumbers()
        {
            ...
            {
                userNumbers[i] = PositiveIntInput(string.Format("Please enter {0} of {1} positive integers --> \t", i + 1,
                    userNumbers.Length));
            }
        ...
        static bool Modulus(int number)
        {
            return number % 2 == 0;
        }
        
        // Test user input as a positive integer.
        static uint PositiveIntInput(string text)
        {
            bool testOK = false;
            string input;
            uint number;
        ...
        static void PrintList(List<uint> numList, string text)
        {
        ...
        static bool EndProgramMenu()
        {
            ...
            while (!inputValid)
            {
                Console.Write("Run program again? (Y)es or (N)o ==> ");
